...
APACHEDS_INSTANCE=/var/lib/apacheds-2.0.0.AM25/default
...
if [ -n "${BOOTSTRAP_FILE}" ]; then
	...
	cat $BOOTSTRAP_FILE | ldapmodify -v -c -a -h localhost -p 10389 -D 'uid=admin,ou=system' -w secret
fi

trap "echo 'Stopping Apache DS';/opt/apacheds-2.0.0.AM25/bin/apacheds stop default;exit 0" SIGTERM SIGKILL
